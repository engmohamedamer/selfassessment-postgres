...
var app = new Vue({
          ...
          <div class="text-center survey-preloader" style="display:none">
            ...
            ></v-progress-circular>
                ...
                <div class="row sectiontitle report-title" data-html2canvas-ignore="true">
                    <v-col cols="6">
                      <h2 class='mt-0'>{{langEN.report}}</h2>
                    </v-col>
                    ...
                    <v-col cols="6" class='actionBtns'>
                        ...
                        <a class="btn secBtn" :href='"/assessment/default/view?id=" + SurveyId'></i> {{langEN.backToSurvey}}</a>
                    </v-col>
                </div>
                                ...
                                <v-item>
                                    ...
                                    >
                                        <div class="d-flex flex-no-wrap justify-space-between align-center col-12">
                                            ...
                                            <div>
                                                <p class="d-block mb-1 brandPrimColor ">{{langEN.orgName}}: <span class="brandPTextColor">{{theme.organization.name}}</span></p>
                                                <p class="d-block mb-1 brandPrimColor">{{langEN.orgAddress}}: <span class="brandPTextColor">{{theme.organization.address}}</span></p>
                                                <p class="d-block mb-1 brandPrimColor">{{langEN.orgAbout}}: <span class="brandPTextColor">{{theme.organization.about}}</span></p>
                                            </div>
                                            <v-avatar
                                                class="ma-3"
                                                size="125"
                        ...
                        <div class="fullWidth eight columns">
                            <div class="projectFactsWrap " style="justify-content: center;">
                                <div class="item wow fadeInUpBig animated animated" data-number="12" style="visibility: visible;">
                                    ...
                                    <p>{{langEN.correctAction}}</p>
                                    <p id="number1" class="number">{{reportGeneralInfo.survey_corrective_number}}</p>
                                    <span></span>
                                    <p v-if='reportGeneralInfo.survey_corrective_number'>{{langEN.mustFinished}}</p>
                                    <p v-else>{{langEN.doesntExist}}</p>
                                </div>

                                <div class="item wow fadeInUpBig animated animated" data-number="246" style="visibility: visible;">
                                    <i class="fas fa-calendar-day"></i>
                                    <p>{{langEN.assesFillDate}}</p>
                                    ...
                                    {{langEN.expiryDate}} : {{reportGeneralInfo.survey_expired_at}}
                                    </p>
                                </div>
                                ...
                                <div class="item wow fadeInUpBig animated animated" v-if="reportGeneralInfo.total_points" data-number="359" style="visibility: visible;">
                                    ...
                                    <p id="number3" class="number" v-else>{{langEN.withoutPoints}}</p>
                                    <span></span>
                                    <p>
                                    {{langEN.degree}} : {{reportGeneralInfo.gained_score}} 
                                    </p>
                                </div>

                                <div class="item wow fadeInUpBig animated animated" data-number="246" style="visibility: visible;">
                                    <i class="fas fa-clock"></i>
                                    <p>{{langEN.answerTime}}</p>
                                    <p id="number4" class="number">{{reportGeneralInfo.actual_time}} {{langEN.min}}</p>
                                    <span></span>
                                    <p>
                                    {{langEN.assesTimeToFinish}}: {{reportGeneralInfo.survey_time_to_pass}} {{langEN.min}}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <v-card class="mb-5">
                        ...
                        <v-data-table class="mb-5 mt-5"
                            ...
                            :items-per-page="10000">
                                ...
                                {{langEN.noResults}}   {{search}}
                                </v-alert>
                        </v-data-table>
                    </v-card>
                    <v-card class="mt-4 mb-5" v-if="reportGeneralInfo.survey_corrective_number>0" >
                        <v-card-title class="mb-5 search-card">{{langEN.correctAction}}</v-card-title>
                        <template>
                            <v-simple-table class="tablepop">
                                <template v-slot:default>
                                    <thead>
                                        <tr>
                                            <th>{{langEN.question}}</th>
                                            <th>{{langEN.correctAction}}</th>
                                            <th>{{langEN.dateToFinish}}</th>
                                        </tr>
                                    </thead>
                                <tbody>
                ...
                <div id="noreport" style="display:none">
                    <p>  {{langEN.noAccessToReport}} </p><div class="emptyassessment"><div class="img"></div><div class="name"></div></div><div class="emptyassessment"><div class="img"></div><div class="name"></div></div>
        
                    <div class="col-md-12 text-center">
                            <a class="btn btn-primary" href="/assessment/default/view?id=<?= $survey->survey_id ?>">{{langEN.backToSurvey}}</a>        
                        </div>
                </div>
        </div>
    ...
    data: {
        ...
        search: '',
        headers: [
            { text: 'Num', align: '', sortable: true, value: 'qNum' },
            { text: 'Question', align: '', value: 'qText' },
            { text: 'Answer', align: '', value: 'qAnswer' },
            { text: 'Attatchments', align: '', value: 'qAttatchments' },
            { text: 'Gained Points', align: '', value: 'qGainedPoints' },
            // { text: 'النقاط الإجمالية', align: '', value: 'qTotalPoints' },
            { text: 'Notes', align:'', value: 'qCorrectiveActions' },
        ],
        assessmentData: {},
        dialog: false,
        surveyResults: {},

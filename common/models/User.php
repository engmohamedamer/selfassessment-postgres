...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    const ROLE_GOVERNMENT_ADMIN = 'governmentAdmin';
    const ROLE_GOVERNMENT_REP = 'governmentRep';
    ...
    public static function  CountUsers($role,$andWhere='1=1'){
        ...
        return static::find()
            ->join('LEFT JOIN','{{%rbac_auth_assignment}}','{{%rbac_auth_assignment}}.user_id::integer = id')
            ->join('LEFT JOIN','{{%user_profile}}','{{%user_profile}}.user_id::integer = id')
            ->where(['{{%rbac_auth_assignment}}.item_name' => $role])
            ->andWhere($andWhere)
            ->count();
    }
